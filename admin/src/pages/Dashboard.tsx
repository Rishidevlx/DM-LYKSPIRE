  ...
  return (
    <div className="max-w-6xl">
      ...
      <div className="mb-8">
        ...
        <p className="text-[var(--text-secondary)] text-sm mt-1">Welcome back. Here's what's happening with Lykspire.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        {statCards.map((card, i) => (
          ...
          >
            <div className={`bg-[var(--panel-bg)] border border-[var(--border)] rounded-xl p-5 shadow-lg ${card.glow} hover:border-[var(--border)] transition-all group`}>
              <div className="flex items-start justify-between mb-4">
                <div className={`w-10 h-10 rounded-lg ${card.bg} ${card.color} flex items-center justify-center`}>
                  {card.icon}
                </div>
                {card.link && (
                  <Link to={card.link} className="text-[var(--text-secondary)] hover:text-[var(--text-secondary)] transition-colors">
                    <ArrowUpRight size={16} />
                  </Link>
                )}
              </div>
              <p className="text-2xl font-bold text-[var(--text-primary)] mb-0.5">{card.value}</p>
        ...
        >
          <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border)]">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-[var(--text-primary)]">Recent Blogs</h2>
            </div>
          ...
          <div className="divide-y divide-white/[0.04]">
            {isLoading ? (
              <div className="px-5 py-8 text-center text-[var(--text-secondary)] text-sm">Loading...</div>
            ) : recentBlogs.length === 0 ? (
              <div className="px-5 py-8 text-center text-[var(--text-secondary)] text-sm">No blogs yet. <Link to="/blogs/new" className="text-indigo-400 hover:underline">Create one →</Link></div>
            ) : (
              recentBlogs.map((blog) => (
                <div key={blog.id} className="flex items-center gap-3 px-5 py-3 hover:bg-[var(--panel-bg)] transition-colors">
                  <div className="w-7 h-7 rounded-lg bg-indigo-500/10 flex items-center justify-center shrink-0">
                    <FileText size={13} className="text-indigo-400" strokeWidth={1.8} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-[var(--text-secondary)] font-medium truncate">{blog.title}</p>
                    <p className="text-xs text-[var(--text-secondary)] flex items-center gap-1 mt-0.5">
                      <Clock size={11} />
                      {blog.publish_date ? new Date(blog.publish_date).toLocaleDateString() : 'Draft'}
                    </p>
                  </div>
                  <Link to={`/blogs/edit/${blog.id}`} className="text-[var(--text-secondary)] hover:text-indigo-400 transition-colors">
        ...
        >
          <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border)]">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-[var(--text-primary)]">Recent Enquiries</h2>
            </div>
          ...
          <div className="divide-y divide-white/[0.04]">
            {isLoading ? (
              ...
              <div className="px-5 py-8 text-center text-[var(--text-secondary)] text-sm">No enquiries yet.</div>
            ) : (
              recentContacts.map((contact) => (
                <div key={contact.id} className="flex items-center gap-3 px-5 py-3 hover:bg-[var(--panel-bg)] transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-[var(--text-secondary)] font-medium truncate">{contact.name}</p>
